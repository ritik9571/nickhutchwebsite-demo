...
(function(root, factory) {
    ...
    function onDOMReady(callback) {
        if (isDOMReady()) {
            ...
            var evtListener = function evtListener(event) {
    ...
    onDOMReady(function() {
        var _frame = function prepareIframe() {
                ...
                frame.setAttribute('owner', 'archetype'); // HUBSPOT CHANGES START
                // Monkey patching to address issue around iframe not having title for a11y  https://hubspot.slack.com/archives/CB0Q5H56U/p1610473020094600

                frame.setAttribute('title', 'archetype'); // HUBSPOT CHANGES END
        ...
        var fnToString = _safeWindow.Function.prototype.toString,
            ...
            reNative = new RegExp('^' + // Coerce `Object#toString` to a string
                String(toString) // Escape any special regexp characters
                .replace(/[.*+?^${}()|[\]\/\\]/g, '\\$&') // Replace mentions of `toString` with `.*?` to keep the template generic.
        ...
        function isNative(value) {
            ...
            return fnStr ? // Use `Function#toString` to bypass the value's own `toString` method
                // and avoid being faked out.
                reBound.test(fnStr) || reNative.test(fnStr) : // Fallback to a host object check because some environments will represent
        ...
        lib.getNativeMethod = function(path, bindContext) {
            ...
            if (!obj) {
                ...
                obj = getMethodAtPath(path, _entryPoints.safe, obj.context); // try again
        ...
        lib.isNative = function(pathOrMethod) {
            return typeof pathOrMethod === 'string' ? pathIsNative(pathOrMethod) : isNative(pathOrMethod);
        };
        /**
        ...
        lib.getWindow = function() {
            return _entryPoints.safe.window;
        }; // HUBSPOT CHANGES END
        // var lib = {
        //
        // 	/**
